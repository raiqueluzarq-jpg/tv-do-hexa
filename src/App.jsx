...
const getTVSize = (tv) => {
  const size = Number(String(tv?.Tamanho || "").replace(/[^0-9]/g, ""));
  return Number.isFinite(size) ? size : 0;
};

const hasAnyLink = (tv) =>
  Boolean(tv?.["Link ml"] || tv?.["Link amazon"] || tv?.["Link shopee"]);

const getBestLink = (tv) =>
  tv?.["Link ml"] || tv?.["Link amazon"] || tv?.["Link shopee"] || "#";

const getImageUrl = (tv) =>
  tv?.Imagens || tv?.Imagem || tv?.["Imagem URL"] || "";
...
export default function App() {
    ...
    async function loadTVs() {
      try {
        ...
        if (!Array.isArray(data)) {
          throw new Error("A planilha não retornou uma lista de TVs.");
        }
  ...
  const allQuestions = [
    {
      ...
      icon: "📺",
      options: [
        { label: "Quero a maior TV possível", value: "📏 Quero a maior tela possível", icon: "📏" },
        { label: "Quero a TV ideal pro meu ambiente", value: "🏠 Tamanho ideal para meu ambiente", icon: "🏠" },
      ],
    },
    {
      key: "distance",
  ...
  const getDisplayName = (tv) => {
    const brand = safeText(tv?.Marca);
    const size = safeText(tv?.Tamanho);
    const model = safeText(tv?.Modelo, "TV recomendada");
    ...
    return { brand, model: modelWithoutBrand || model, size };
  };

  const scoreTV = (tv) => {
    let score = 0;
    const futebol = normalizeScore(tv?.Futebol);
    const filmes = normalizeScore(tv?.Filmes);
    const games = normalizeScore(tv?.Games);
    const salaClara = normalizeScore(tv?.["Sala Clara"]);
    const coresVivas = normalizeScore(tv?.["Cores Vivas"]);
    const coresNaturais = normalizeScore(tv?.["Cores Naturais"]);
    const compatibilidade = normalizeScore(tv?.["Compatibilidade Geral"]);
  ...
  const getCompatibility = (tv) => {
    ...
    const percent = Math.round(((tv?.score || 0) / bestScore) * 96);
    return Math.max(58, Math.min(98, Number.isFinite(percent) ? percent : 58));
  };
  ...
  const getRecommendations = () => {
    ...
    const bestImage = [...rankedTVs].sort((a, b) => {
      const aImage = normalizeScore(a?.Filmes) + normalizeScore(a?.["Cores Naturais"]) + normalizeScore(a?.["Cores Vivas"]);
      const bImage = normalizeScore(b?.Filmes) + normalizeScore(b?.["Cores Naturais"]) + normalizeScore(b?.["Cores Vivas"]);
      return bImage - aImage || (b.score || 0) - (a.score || 0);
    })[0];

    const cheapestRecommended =
      [...rankedTVs]
        .filter((tv) => getCompatibility(tv) >= 70)
        .sort((a, b) => normalizePrice(a?.["Preço Médio"]) - normalizePrice(b?.["Preço Médio"]) || (b.score || 0) - (a.score || 0))[0] || rankedTVs[0];
  ...
  const handleAnswer = (option) => {
    ...
    setTimeout(() => setStep((previous) => previous + 1), 140);
  };
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .brand-title span {
          color: var(--gold-500);
          text-shadow: 0 10px 30px rgba(234, 179, 8, 0.20);
        }
        ...
        .progress {
          height: 100%;
          border-radius: 999px;
          background: linear-gradient(90deg, var(--green-700), var(--green-500), var(--gold-400));
        ...
        .question-icon {
          ...
          border: 1px solid rgba(5, 150, 105, 0.12);
          box-shadow: 0 18px 35px rgba(5, 150, 105, 0.12);
          font-size: 34px;
          margin-bottom: 16px;
        }
        ...
        .option-card:hover {
          ...
          box-shadow: 0 22px 48px rgba(5, 150, 105, 0.13);
        ...
        .warning {
          ...
          background: linear-gradient(145deg, #fef9c3, #fff7ed);
          color: #854d0e;
          font-weight: 850;
          line-height: 1.4;
          box-shadow: 0 14px 35px rgba(234, 179, 8, 0.10);
        }
        ...
        .image-box {
          aspect-ratio: 16 / 9;
          border-radius: 22px;
          background: radial-gradient(circle at center, rgba(16, 185, 129, 0.10), transparent 52%), #f8fafc;
        ...
        .gold-btn { background: linear-gradient(135deg, var(--gold-500), #fde047); color: var(--zinc-900); }

        .extra-card, .cta-card { padding: 28px; }
        ...
        <div className="shell">
          <header className="hero">
            ...
            <h1 className="brand-title">
              TV do <span>Hexa</span>
            </h1>

            <p className="subtitle">
              Responda algumas perguntas e descubra TVs que combinam com seu
              ambiente, seu uso e seu orçamento.
            </p>
              ...
              >
                <div className="glass-card result-hero">
                  ...
                  <p className="result-copy">
                    Com base nas suas respostas, essas são as TVs mais indicadas
                    para você.
                  </p>
                </div>
                ...
                {rankedTVs[0]?.fallbackMode && (
                  <>
                    <div className="warning">
                      ⚠️ Não encontramos TVs no tamanho ideal para esse orçamento.
                      Mostramos as melhores opções disponíveis dentro da sua faixa
                      de preço.
                    </div>
                    <div style={{ height: 20 }} />
                  </>
                ...
                {loading ? (
                  ...
                  <div className="glass-card empty-card">
                    ...
                    <p className="subtitle">
                      Tente aumentar a faixa de preço ou adicionar mais links na
                      planilha.
                    </p>
                  </div>
                ) : (
                  <>
                    <div className="results-grid">
                      {getRecommendations().map((item) => {
                        ...
                        return (
                          <article
                            key={item.title + "-" + safeText(tv?.Modelo)}
                            className="glass-card tv-card"
                          >
                            ...
                            <span className="compat-pill">
                              ✨ {getCompatibility(tv)}% compatível
                            </span>
                            ...
                            <p className="meta">
                              {safeText(tv?.Tecnologia, "Tecnologia")} •{" "}
                              {safeText(tv?.Sistema, "Sistema")} •{" "}
                              {safeText(tv?.Hz, "60")}Hz
                            </p>

                            <p className="price-label">Preço médio estimado</p>
                            <p className="price">
                              R$ {formatMoney(tv?.["Preço Médio"])}
                            </p>

                            <p className="price-note">
                              Valores podem variar conforme promoção, estoque e loja.
                            </p>
                    ...
                    <div className="glass-card extra-card">
                      <h3 className="section-title">
                        Outras TVs que também combinam com você
                      </h3>
                            ...
                            >
                              <div>
                                ...
                                <p className="extra-meta">
                                  {display.size} polegadas •{" "}
                                  {safeText(tv?.Tecnologia, "Tecnologia")} •
                                  preço médio R$ {formatMoney(tv?.["Preço Médio"])}
                                </p>
                              </div>

                              <div className="extra-percent">
                                <strong>{getCompatibility(tv)}%</strong>
                ...
                <div className="glass-card cta-card">
                  <h3 className="section-title">
                    Quer receber alertas de promoção?
                  </h3>

                  <p className="subtitle">
                    Entre nos canais do TV do Hexa e veja ofertas atualizadas.
                  </p>

                  <div className="cta-buttons">
                    <a href={WHATSAPP_LINK} target="_blank" rel="noreferrer" className="primary-btn">
                      WhatsApp
                    </a>

                    <a href={TELEGRAM_LINK} target="_blank" rel="noreferrer" className="primary-btn telegram-btn">
                      Telegram
                    </a>
                    ...
                    <button
                      ...
                      onClick={() => {
                        const text =
                          "Minha TV ideal no TV do Hexa foi: " +
                          (rankedTVs[0]?.Modelo || "uma TV incrível") +
                          "! Faça o teste também.";
                  ...
                  <button type="button" onClick={resetQuiz} className="reset-btn">
                    Refazer quiz
                  </button>
                </div>
              </motion.section>
